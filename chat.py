...
st.set_page_config(page_title="SeaBuddy", layout="wide")
...
llm = ChatOpenAI(api_key=OPENAI_API_KEY, model="gpt-4o", temperature=0.5, streaming=True)
...
st.markdown("<h1 class='saybuddy-title'>⛵ SeaBuddy</h1>", unsafe_allow_html=True)
...
if st.session_state.page == "home":
    ...
    st.markdown("""
        ...
        .saybuddy-message .message-box {
            ...
            color: #ffffff !important; /* Force white color for all text */
            padding: 15px;
            border-radius: 15px;
            font-size: 16px;
            line-height: 1.5;
        ...
        .saybuddy-message .message-box * {
            color: #ffffff !important; /* Ensure all nested elements are also white */
        }
        </style>
        """, unsafe_allow_html=True)

    st.markdown("<h1 class='title'>⚓ SayBuddy - Your Mental Health Chatbot 🚢</h1>", unsafe_allow_html=True)
    st.markdown("<p class='subtitle'>Welcome to SayBuddy! I'm here to assist you with any mental health-related questions or concerns. Feel free to share your thoughts or ask for advice.</p>", unsafe_allow_html=True)
    ...
    if st.button("Ask SayBuddy"):
        if user_input.strip() != "":
            with st.spinner("SayBuddy is thinking..."):
                ...
                st.session_state.conversation_history.append(("SayBuddy", response))
        else:
            st.warning("Please enter a question or concern related to mental health.")

    # Display conversation history
    st.markdown("<div class='conversation-container'>", unsafe_allow_html=True)
    for speaker, message in st.session_state.conversation_history:
        message_class = "human-message" if speaker == "You" else "saybuddy-message"
        st.markdown(
            f"<div class='message-container {message_class}'><div class='message-row'><div class='message-box'><strong>{speaker}:</strong> {message}</div></div></div>",
            unsafe_allow_html=True
        )
    st.markdown("</div>", unsafe_allow_html=True)
